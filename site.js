...
(function () {
  ...
  function ensureWhatsNew() {
    ...
    if (cta && !cta.querySelector('a.ni-whatsnew')) {
      ...
      a.className = 'ni-whatsnew'; a.href = '/whats-on'; a.textContent = "What's On";
    ...
    if (ov && !ov.querySelector('a.ni-whatsnew')) {
      ...
      m.className = 'ni-whatsnew'; m.href = '/whats-on'; m.textContent = "What's On";
  ...
  function galArrow(dir) {
    ...
    b.setAttribute('aria-label', dir === 'prev' ? 'Previous photos' : 'More photos');
    b.innerHTML = dir === 'prev' ? '\u2039' : '\u203a';
    b.style.cssText = 'position:absolute;top:50%;transform:translateY(-50%);' + (dir === 'prev' ? 'left:6px' : 'right:6px') +
      ';z-index:6;width:44px;height:44px;border-radius:50%;border:none;cursor:pointer;background:rgba(20,20,20,.82);color:#e8c97a;' +
  ...
  function enhanceHeader() {
    ...
    if (!navDiv) return;
    // swap the boxed navy logo for the clean transparent navy mark
    var logoImg = row.querySelector('a.group img');
    if (logoImg && !/logo-mark-blue/.test(logoImg.getAttribute('src') || '')) logoImg.src = '/logo-mark-blue.png';
